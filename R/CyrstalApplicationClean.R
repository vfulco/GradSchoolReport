...
CrystalApplicationClean.data.frame <- function(x, ...){
  ...
  bind_rows(mutate(filter(x, `LINE 2` == "A" & `LINE 3` == "A" & `LINE 4` %in% c("Y", "Ys", "Yd", "A")),
                       ...
                       mutate(filter(x, `LINE 3` == "C" | `LINE 2` == "C", is.na(`LINE 4`)),
                              Event = "Cancelled",
                              `Event Date` = `LINE 3 DATE`),
                       mutate(filter(x, `LINE 3` == "C" | `LINE 2` == "C", `LINE 4` %in% c("N", "Ns", "Nd", "Y", "Ys", "Yd", "A")),
                              Event = "Cancelled",
                              `Event Date` = `LINE 3 DATE`),
                       mutate(filter(x, `LINE 4` == "C"),
                              Event = "Cancelled",
                              `Event Date` = `LINE 4 DATE`),
