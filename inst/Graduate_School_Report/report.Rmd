...
\vspace{1in}
\Large\begin{center}
Degree: `r params$degree` \hspace{0.5in}
...
\begin{tikzpicture}[every annotation/.style = {draw, fill = white, font = \Large,}]
  ...
  node[root] {`r flowChartData$Total` \\ Applications \\ in `r maxYear` \\ Calendar Year} [clockwise from=20]
    child[concept color = BaylorGreen!50!BaylorGold] {
      node {`r flowChartData$Offers` Offers \\ (`r flowChartData$OffersPer` \% \\ accepptance \\ rate)} [clockwise from=10]
        child[concept color = BaylorGold] { node {`r flowChartData$Enrolled` \\ enrolled \\ (`r flowChartData$EnrolledPer`\% yield)}} 
        child[concept color = BaylorGold] { node {`r flowChartData$Declined` \\ enrolled \\ (`r flowChartData$DeclinedPer`\%)}} 
    }
    child[concept color=BaylorGreen!50!BaylorGold] {
...
file.copy(path, paste0("temp/", files))
    }
...
```{r, eval = evalcheck, results = 'asis'}
  files %>% l_ply(function(file){  
     cat('\\includepdf[pages = -]{', 'temp/', file, '}', sep = "")
    cat('')
   })
```
...
if(is.null(input$fileALL)){
  ...
  files <- inFile$name
  file.copy(inFile$datapath, paste0("temp/", inFile$name))
}
